...
- Roadmap items 1 (`examples/basic-site/`), 2 (`refscilink.config.json`), 3 (`tools/install_refscilink.mjs`) and 4 (`package.json` npm scripts) are validated.
...
- Static checks confirmed `refscilink.config.json` source/output/display/theme/language settings, local installer syntax, local static server syntax, npm script execution, navigation integration, French generated pages, localized detail metadata, stable detail links and external-link safety guards.
...
- next likely target is the developer quick start documentation.
