...
- The latest `examples/basic-site` module test passed with 80 automated checks.
...
- Roadmap items 1 (`examples/basic-site/`), 2 (`refscilink.config.json`), 3 (`tools/install_refscilink.mjs`), 4 (`package.json` npm scripts), 5 (`README.md` quick start), 6 (`tools/theme_detector.mjs`), 7 (`theme_refscilink.json` editable overrides), 8 (`tests/theme_detection.test.mjs`) and 9 (mixed-format Markdown extraction) are validated.
...
- `npm run test:basic-site` passed with 80 checks.
...
- Static checks confirmed `refscilink.config.json` source/output/display/theme/language settings, README quick-start coverage, automatic theme detection, dedicated theme detection fixtures, editable theme runtime overrides, mixed-format Markdown extraction, local installer syntax, local static server syntax, npm script execution, navigation integration, French generated pages, localized detail metadata, stable detail links and external-link safety guards.
...
- next likely target is dedicated extraction tests.
