...
Generated JSON metadata must distinguish `module_version` from `schema_version`, tolerate legacy `version` fields and never erase scientific validation data during migration.

## Post-install validation checklist

Post-install validation checklist is specified in:

```text
skills/contracts/post_install_validation_checklist.md
```

Final reports must distinguish `pass`, `fail`, `warning` and `manual_review_required`, and must not hide scientific uncertainty or manual-review needs.
